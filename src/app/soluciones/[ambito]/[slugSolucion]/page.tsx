...
import { Props as SolucionSeoProps } from "@/components/seo/SolucionSeo"; // Asegúrate de exportar el tipo
...
const SolucionAmbitoPage = () => {
	...
	const [seoData, setSeoData] = useState<Partial<SolucionSeoProps> | null>(null);
	...
	useEffect(() => {
		const fetchSolucion = async () => {
			try {
				...
				// SEO dinámico basado en los datos recibidos
				setSeoData({
				title: data.title,
				description: data.subtitle ?? data.descripcion ?? '',
				slug: data.slug ?? slugSolucion,
				ambito: data.slug_ambito,
				sector: data.slug_sector ?? undefined,
				image: data.imagenOg ?? 'https://www.normapro.es/images/og-default.jpg',
	...
	return (
		<>
			...
			{seoData?.title && seoData?.description && seoData?.slug && seoData?.ambito && (
				
				<SolucionSeo {...(seoData as SolucionSeoProps)} />
			)}
						...
						<div className="col-span-8">
							...
							<div className="mt-12 space-y-10">
								...
								{solucion?.casosdeuso !== undefined && solucion?.casosdeuso?.length > 0 && (
									<ul className="list-disc ml-5 text-[15px] text-[#010D3D]">
										{solucion.casosdeuso.map((caso, index) => (
											<li key={index}>{caso.description}</li>
										))}
									</ul>
								)}
